...
export class HomePage implements OnInit {
  ...
  try {
    ...
    if (!user || !token) {
      console.warn('❗ Respon tidak lengkap:', response);
      alert('Respon tidak lengkap dari server');
      return;
    }

    localStorage.setItem('user', JSON.stringify(user));
    localStorage.setItem('token', token);
    console.log('🔐 Token login disimpan');
    ...
    if (this.platform.is('cordova') || this.platform.is('android')) {
      try {
        ...
        console.warn('⚠️ Gagal ambil FCM token:', fcmError);
      }
    } else {
    ...
    alert(`Login gagal: ${error.status || ''} - ${error.message || 'Terjadi kesalahan'}`);
  } finally {
    this.loadingLogin = false;
  }
}
